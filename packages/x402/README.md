...
| `facilitator` | x402.org on Base Sepolia only | `{ url, headers?: () => Promise<Record<string, string>> }` for an x402 facilitator's HTTP API — `headers` runs per request (e.g. a CDP JWT) — or your own `X402Facilitator` (`{ verify, settle }`) for a provider with a different API or a choice among several. **Required on mainnet and every other network**; the testnet facilitator is never used silently. The rail holds every implementation to the same contract: reasons are sanitized to `[a-z0-9_]`, `unexpected_verify_error` is 503, a settle without a `0x` transaction hash is unknown, `settlement_pending` is unknown. `x402FacilitatorClient({ url, headers })` is the built-in HTTP implementation, exported so a custom one can delegate to it. |
