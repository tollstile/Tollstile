...
describe('request commitment', () => {
  it('rejects a cheap quote replayed with a larger body, before any charge', async () => {
    ...
    expect(swapped).toMatchObject({ status: 402, handlerRuns: 0, body: { reason: 'quote_mismatch', price: '$1000.00' } });
  ...
  it('binds the query string and method', async () => {
    ...
    expect((await call(gate, { path: '/search?q=b', payment: `test quote=${quote}` })).body.reason).toBe('quote_mismatch');
    expect((await call(gate, { path: '/search?q=a', method: 'DELETE', payment: `test quote=${quote}` })).body.reason).toBe(
      'quote_mismatch',
    );
  ...
  it('binds MCP tool arguments regardless of key order', async () => {
    ...
    expect(await enter({ pages: 500, format: 'pdf' }, payment)).toMatchObject({ reason: 'quote_mismatch' });
  ...
  it('lets a route bind only the fields that affect the price', async () => {
    ...
    expect(swapped.body.reason).toBe('quote_mismatch');
  });
  ...
  it('charges reusable authorizations the current price of each request', async () => {
    ...
    expect((await call(gate, { path: '/translate', body: huge, payment })).body.reason).toBe('insufficient_authorization');
