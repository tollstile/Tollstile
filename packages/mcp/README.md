...
- **Settlement rejected after the tool ran.** x402's MCP transport says the server should then return the payment error instead of the tool's output. `pass.complete()` resolves to a receipt without saying whether settlement was rejected, left `unknown`, or released, and withholding output on an `unknown` settlement that later reconciles as settled would charge for a service never delivered. So today the output is returned **without a receipt**; the charge is recorded as `failed/completed` and a `SETTLEMENT_REJECTED` error event is emitted. Returning a payment-required result instead needs core to report the settlement result from `complete()`.
