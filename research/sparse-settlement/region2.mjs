...
for (const K of [10, 25, 100]) {
  ...
  for (const [lab, Tc] of CAPS) {
    for (const R of [10, 50]) {
      const inr = os.filter(r => Math.max(r.p, Math.min(Tc, r.p * r.n / K)) / r.p >= R);
      const all = rows.filter(r => Math.max(r.p, Math.min(Tc, r.p * r.n / K)) / r.p >= R);
...
for (const [lab, Tc] of CAPS) console.log(`  ${lab}: p <= $${(Tc/10).toFixed(4)} → one-shot resources meeting both: ${os.filter(r=>r.n>=250 && r.p<=Tc/10).length}`);
...
svg += `<line x1="${lx(250)}" y1="${Tp}" x2="${lx(250)}" y2="${H-B}" stroke="#666" stroke-dasharray="3 3"/><text x="${lx(250)+5}" y="${Tp+12}" fill="#666" font-size="10">n = k·10 = 250</text>`;
...
for (const [i,[lab,Tc,col]] of CAPS.entries()) svg += `<text x="${W-R}" y="${H-B-8-i*15}" text-anchor="end" font-size="11" fill="${col}">inside ${lab.split(' —')[0]}: ${os.filter(r=>r.n>=250&&r.p<=Tc/10).length} resources</text>`;
svg += `</svg>`;
