...
*Draft v0.7 — 21 September 2026 (reference rail added the same day)*
...
**Onboarding — the bound the classical literature could ignore.** Rivest, Peppercoin, Orchid and Livepeer all assume the payer has an account, escrow or deposit; onboarding is outside the model. In x402 it is inside it, and asymmetric: the default `exact` path costs the buyer **no** transaction; every Permit2 path costs a first-time buyer one approval. Pricing that approval at one transaction unit (an ERC-20 `approve` is ~46k gas, comparable to a transfer), and writing φ for the share of a resource's payers who are first-time Permit2 users:

  sparse beats `exact` on total gas  ⇔  φ < (*n*/*u*) · (1 − *p*/*T*_eff)

For a low-recurrence resource *n*/*u* ≈ 1, so the condition is roughly φ < 1 − *p*/*T*_eff: sparse wins only to the extent that it actually reduces settlements. Where variance forces *T*_eff = *p*, it cannot win at any φ.
...
**Onboarding inside the region.** For the 25 resources, the break-even first-time share φ* = (*n*/*u*)(1 − *p*/*T*) has median 0.97: sparse beats per-call settlement unless essentially every buyer is new. Six of the 25 beat it even at φ = 1.

**Onboarding in the cell the mechanism is for.** Across all 4,716 low-recurrence sub-cent resources, **4,669 have φ\* = 0**: the variance bound forces *T* = *p*, the ticket reduces nothing, and per-call settlement is cheaper at any share of returning buyers. Eleven beat `exact` with all-new buyers. In the cell it was built for, sparse settlement is `exact` with extra steps for 99% of resources.
...
**Contract-path gas.** A sparse settlement is a Permit2 proxy call with a witness, not a bare EIP-3009 transfer, and channel claims and sweeps are contract calls too; pricing them at 1× a transfer favours them. At a 1.5× / 2× / 3× premium on every contract-path transaction, always-channel becomes $332 / $442 / $663, always-sparse $191 / $255 / $382, the oracle $112 / $127 / $153 — and the margin of selection over the best single regime *widens*, to 1.70× / 2.00× / 2.50×, because `exact` is the only regime the premium does not touch. Calls routed to sparse stay at 44–45% throughout.
...
Other limitations: gas is an assumption; the channel model is the per-receiver worst case (§6); client caps are three data points, not a survey; recurrence from unique addresses misreads rotated and shared addresses; sparse excludes EIP-3009 structurally and ERC-1271 unless added; the legal question is open.
...
3. **Verifier contract**, forked from `x402UptoPermit2Proxy` to enforce invariants 1–3. Audit.
4. **The three measurements of §11**, then re-run §5. The region is a function of the market, not the mechanism.
