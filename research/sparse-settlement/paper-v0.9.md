...
*Draft v0.9 — 22 September 2026 (exact variance bound; reference rail and verifier contract with measured gas)*
...
**Merchant variance.** Wins are Binomial(*n*, *q*) with *q* = *p*/*T*, so realized revenue has coefficient of variation

  CV = √(*nq*(1 − *q*)) / (*nq*) = √((1 − *q*)/(*nq*)) = **√((*T*/*p* − 1) / *n*)**

and CV ≤ *c* holds exactly when

  **_T_ ≤ _p_ (1 + *n*/*k*),  *k* = 1/*c*²**

(*k* = 25 for CV ≤ 20%, 100 for 10%). Earlier drafts of this work used the small-*q* approximation CV ≈ 1/√(*nq*), giving *T* ≤ *p·n*/*k* and dropping the leading 1. The correction is one term and it matters at both ends: a ≥10× reduction needs *n* ≥ *k*(*R* − 1) = 225 calls, not 250; and at *n* = 1 the bound is *T* ≤ 1.04*p*, so variance never forces *T* exactly to *p* — it forces the reduction to a few percent, which is a different statement and the one §5 now makes. Everything below uses the exact form.
...
**The combined bound.**
  ...
  *T* = min( *p*(1 + *n*/*k*), *T*_client, *T*_policy ),  reduction *R* = *T*/*p*,  subject to φ < (*n*/*u*)(1 − *s·p*/*T*)/*a*

where *s* and *a* are the gas of one sparse settlement and of one Permit2 approval, each relative to one `exact` settlement — measured in §6 as *s* = 0.91–1.11 and *a* = 0.54, not assumed to be 1. A reduction of at least *R* needs all three of *n* ≥ *k*(*R* − 1), *T*_client ≥ *Rp* and *T*_policy ≥ *Rp*. That is a region in the (*n*, *p*) plane.

## 5. The region

**Proposition.** *Take a resource priced p with n calls and u distinct payers in the period, a merchant tolerance CV ≤ c (k = 1/c²), a client cap T_client and a merchant cap T_policy, and let T = min(p(1 + n/k), T_client, T_policy). Then a sparse ticket reduces settlements by a factor of exactly T/p; it reaches a factor R iff*

  *n ≥ k(R − 1)  **and**  T_client ≥ Rp  **and**  T_policy ≥ Rp;*

*and it reduces total gas relative to per-call `exact` settlement iff the resource's first-time-Permit2 share φ satisfies*

  *φ < (n/u)(1 − s·p/T)/a,*

*with s, a the sparse-settlement and Permit2-approval gas, each relative to one `exact` settlement. In particular the gas condition is unsatisfiable at any φ when T/p ≤ s: a ticket that does not reduce settlements by more than the settlement's own gas premium can never pay for itself.*
...
|---|---|---|---|---|
| $1 (x402 reference default) | *n* ≥ 225, *p* ≤ $0.10 | **25** of 11,630 low-recurrence | 14,839 | 3.0% |
| $0.05 (agent-wallet doc example) | *n* ≥ 225, *p* ≤ $0.005 | **22** | 12,930 | 2.7% |

**Robustness.** The exact bound moves the volume threshold from 250 to 225 calls and admits **the same 25 resources** — the index has none between the two, so the correction changes the arithmetic and not the answer. Relaxing the tolerance to *k* = 10 (CV 32%) admits 39; tightening to *k* = 100 (CV 10%) admits 4; demanding a 50× reduction admits none. Against the call definition (§1.2 i), the region is stable — 25 → 23 → 22 → 22 at *f* = 1, 0.5, 0.25, 0.1 — because its members are resources with hundreds of distinct payers, whose counts are not inflated by repeated unpaid probes.
...
So the earlier draft's "variance forces *T* = *p*" was false as stated and true in effect: the median resource can carry a ticket, and that ticket saves it four percent of its settlements. Against the measured gas premium *s* = 1.11 this is worse than nothing — by the Proposition the gas condition is unsatisfiable whenever *T*/*p* ≤ *s*, and **3,648 of 4,716 resources are below that line at any φ**. Sixty-three beat `exact` even with every buyer new. In the cell it was built for, sparse settlement is `exact` with extra steps for three quarters of resources and a rounding error for most of the rest.
...
| low-recurrence, ≥ $0.01 | $3,614 | $28 | $60 | $26 | $26 |
| repeat, sub-cent | $365 | $231 | $23 | $19 | $15 |
| repeat, ≥ $0.01 | $9,955 | $660 | $33 | $36 | $24 |
| **index** | **$14,032** | **$975** | **$221** | **$103** | **$86 (oracle)** |

*(sparse priced at the measured gas ratios of §6: 1.11 of an `exact` settlement for a low-recurrence resource's buyers, 0.91 for a repeat resource's)*

**$86 is an oracle, not a selector — and an oracle cannot lose.** It picks, for each resource, the regime that was cheapest over the same thirty days it is scored on. Under any additive cost model Σᵢ minᵣ Cᵢᵣ ≤ minᵣ Σᵢ Cᵢᵣ holds identically, so "the oracle beats every fixed regime" is arithmetic, not evidence. We report it only as the bound. The evidence is what a **deployable rule** does — one with thresholds a merchant can set in advance and hold fixed: *mean recurrence > 2 → channel; else ≥ 100 payers and p ≤ $0.10 → sparse; else `exact`*. **Evaluated retrospectively** on the same window (its inputs *n*, *u* are the month's realised values; a true out-of-sample test needs the second snapshot of §11), it costs **$107**, 1.28× the oracle's $84 at a uniform gas ratio. Fed *forecasts* of *n* and *u* perturbed by lognormal noise instead of realised values, it costs $90 / $99 / $107 at typical errors of ±65% / ×÷2.7 / ×÷4.5 — it degrades to its own fixed-threshold cost as the forecast degrades, which is the right failure mode.
...
| *f* | rule beats best single regime | best-single / rule (min / median / max) | rule / oracle (min – max) | best single regime, over the sweep |
|---|---|---|---|---|
