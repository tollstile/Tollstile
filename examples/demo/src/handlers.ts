...
const CORPUS: readonly { topic: readonly string[]; text: string; source: string }[] = [
  ...
  { topic: ['ferry'], text: 'The Takeshiba to Oshima jetfoil is cancelled when swell exceeds 2.5 metres.', source: 'tokai-kisen/service-rules' },
  { topic: ['refund'], text: 'A cancelled sailing is refunded in full; a missed connection is not.', source: 'tokai-kisen/tariff' },
...
export function research(question: string): { answer: string; sources: readonly string[] } {
  const words = question.toLowerCase().split(/[^a-z]+/).filter((word) => word.length > 2);
  ...
  const conclusion =
    hits.length >= 3
      ...
      : hits.length === 2
        ...
        : '';
  return { answer: `${hits.map((card) => card.text).join(' ')}${conclusion}`, sources: hits.map((card) => card.source) };
}
