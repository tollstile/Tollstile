...
async function ask(detection: Detection, image: Uint8Array, target: string, apiKey: string, fetcher: typeof globalThis.fetch): Promise<Verdict> {
  ...
  const dataUri = `data:image/png;base64,${png(cut.pixels, cut.width, cut.height).toString('base64')}`;
    ...
    const response = await fetcher(ENDPOINT, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: `You check one cropped region of an aerial image. Answer with exactly one word, "yes" or "no": is the region ${target}? Answer no for anything else, however similar.`,
          },
